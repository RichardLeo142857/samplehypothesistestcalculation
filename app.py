...
if pred_low <= user_prediction <= pred_high:
    st.success("✅ 预测值落在置信区间 → 预测值合理")
else:
    st.error("❌ 预测值落在置信区间外 → 预测值不合理")
...
st.markdown(f"双尾检验 {conf_choice} 置信区间 = ({conf_low_two:.4f}, {conf_high_two:.4f})")
if conf_low_two <= mean <= conf_high_two:
    st.info("双尾：样本均值落在置信区间 → 没有足够证据证明均值与 μ₀ 不同")
else:
    direction = "大于 μ₀" if mean > mu0 else "小于 μ₀"
    st.error(f"双尾：样本均值落在置信区间外 → 有足够证据证明均值{direction}")
...
# 单尾检验
if mean > mu0:
    t_crit_one_val = stats.t.ppf(1 - alpha, df)
    conf_low_one = mu0
    conf_high_one = mu0 + t_crit_one_val*S/np.sqrt(n)
    tail_text = "右尾 (μ > μ₀)"
else:
    t_crit_one_val = stats.t.ppf(1 - alpha, df)
    conf_low_one = mu0 - t_crit_one_val*S/np.sqrt(n)
    conf_high_one = mu0
    tail_text = "左尾 (μ < μ₀)"

st.markdown(f"单尾检验 {conf_choice} 置信区间 ({tail_text}) = ({conf_low_one:.4f}, {conf_high_one:.4f})")
if conf_low_one <= mean <= conf_high_one:
    st.info(f"单尾：样本均值落在置信区间 → 没有足够证据证明均值{tail_text}")
else:
    st.error(f"单尾：样本均值落在置信区间外 → 有足够证据证明均值{tail_text}")
