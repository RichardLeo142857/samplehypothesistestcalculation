...
if data2 is None or len(data2) < 2:
    ...
    # --- show table of common alpha critical t values and corresponding mu-critical boundaries (based on mu0) ---
    st.markdown("### 常用显著性水平对应的 critical t 值 与 μ 轴临界边界（以 μ₀ 为中心）")
    alpha_list = [0.10, 0.05, 0.01]
    rows = []
    for a in alpha_list:
        tcrit_one = stats.t.ppf(1 - a, df2)  # for right-tail boundary (positive); left-tail boundary is negative of this
        if tail_dir == "right":
            mu_crit = mu0 + tcrit_one * S2 / np.sqrt(n2)
        else:
            mu_crit = mu0 - tcrit_one * S2 / np.sqrt(n2)
        rows.append({"alpha": f"{int(a*100)}%", "t_crit_one": f"{tcrit_one:.4f}", "mu_critical_boundary": f"{mu_crit:.4f}"})
    st.table(rows)
